...
# Advantages: lower variance, high accuracy, good in large datasets, uses decorrelated trees unlike bagging so does not overfit
...
data <- read.csv("Data/Data 2.csv")
summary(data)

# Clean data
data["fwi_99_flag"][data["fwi_99_flag"] == TRUE] <- 1 #Convert T/F to 1/0 for easier model handling
data["fwi_99_flag"][data["fwi_99_flag"] == FALSE] <- 0 #^
...
bestmtry <- 10
#10 using old data, 8 using new data
